...
namespace RendimientoPlantaWS.Repositories
{
    public class JornadaRepository
    {
        ...
        public async Task Insert(IEnumerable<Jornada> jsonJornadas)
        {
            foreach (Jornada value in jsonJornadas)
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("PR_UPD_MAN_JORNADA", sql))
                    {
